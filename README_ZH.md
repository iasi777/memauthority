...
你也可能尝试过维护记忆库，但随着内容积累，它变得越来越臃肿、杂乱，甚至开始干扰 Agent 的正常判断
...
你平时只需要用简短自然的指令与 Agent 交互：

### 1. 开始任务时按需读取
*Agent 会根据任务实际情况自行决定是否调用，机制类似 MEMORY.md ，也可以指定 Agent 读取相应的记忆*
> **“先看看这个项目的 MemAuthority，需要什么再读什么”**

Agent 会选择最短、最合适的读取路径，MemAuthority 没有强制指定读取的方式：已知具体位置就直接读取，不知道位置时再检索，需要快速接手项目整体状态时再看 `handoff`。如果当前对话本身已经有足够上下文，就完全没有必要调用 MemAuthority
...
同时，Agent 不会仅仅因为某句话“听起来很重要”，就擅自将其提升为长期 `rules` 真正稳固的规则、接手状态和踩坑教训，应在长期的实际工作中逐步沉淀
...
---
## 如何避免MemAuthority中不必要的记忆进入当前对话上下文？
...
目标很简单：不要让无关记忆进入当前对话上下文，把“需要多少证据才够”留给正在执行任务的 Agent 判断
...
## 如何避免MemAuthority越来越臃肿

Git 负责忠实记录 Authority 的完整演变历史，随时可以追溯记忆，但 Agent 实际工作中并不会读取；Agent 只读取活跃的记忆，活跃的记忆可以随意修订，Git 保证可恢复
