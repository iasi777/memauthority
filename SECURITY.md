...
The current release is `v1.1.0` on the v1 compatibility line. Security fixes that preserve the frozen surface are patch-version changes under the frozen SemVer policy; incompatible security changes require the frozen major-version path.

## Deployment safety

Follow the current frozen [transport/auth](docs/contract/v1.1/transport-auth.md), [managed runtime](docs/contract/v1.1/managed-runtime.md), and [structured error](docs/contract/v1.1/structured-errors.md) contracts. Do not weaken loopback/authentication, clean-snapshot, fencing/CAS, state-containment, or fail-closed checks to work around deployment issues.
