# Public MemAuthority CLI contract — 1.3.0
...
`memauthority version` and `memauthority --version` print exactly `memauthority 1.3.0` plus newline. The compatibility forms `v-memory version` and `v-memory --version` print exactly `v-memory 1.3.0` plus newline. Extra arguments are usage errors (exit 2). See `VERSION-DECISION.md` for the SemVer compatibility policy.
