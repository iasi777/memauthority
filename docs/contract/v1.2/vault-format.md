# Public Vault and Authority contract — 1.2.0
...
Runtime metadata remains optional. Within a runtime manifest, `host_id` is optional user-defined metadata rather than a fixed V-Memory machine enumeration. If a manifest has exactly one environment, omitted `environment_id` is interpreted/canonicalized as `default`; a manifest with multiple environments requires an explicit valid `environment_id` for every environment.
