# Public MCP contract — 1.2.0

The server implementation identity is `V-Memory` / `1.2.0`. `mcp-tools.json` records the complete runtime-enabled tool definitions; `mcp-resource-templates.json` records the optional runtime resource template. Runtime-free startup intentionally exposes a smaller subset.
...
> V-Memory provides project-scoped long-term memory and cross-conversation continuity. Use it when prior project state is needed and is not already available in the current context. It can find and read durable memory and update validated Git-backed Authority. Authority writes must use V-Memory mutation tools, not machine or filesystem tools. Optional declarative runtime metadata is enabled; it describes stored topology and does not prove live runtime health.
...
A Vault with no registered `runtime_resource`, started without `--runtime-enabled`, exposes exactly 13 tools:
...
Runtime capability is enabled when `--runtime-enabled` / `VM_RUNTIME_ENABLED=1` is used, or automatically when the admitted Vault already contains any registered runtime resource. This preserves existing v1.1 runtime Vault behavior across upgrade.
...
Tool names, titles, descriptions, input/output schemas and annotations in `mcp-tools.json`, the conditional exposure rules above, the optional resource-template schema, the implementation version, and the two instruction variants above are the v1.2.0 compatibility surface represented by this snapshot.
